...
RUN CGO_ENABLED=1 GOOS=linux go build -a -installsuffix cgo -ldflags "-extldflags \"-static\" -s -w -X github.com/P3X-118/owncast/config.GitCommit=$GIT_COMMIT -X github.com/P3X-118/owncast/config.VersionNumber=$VERSION -X github.com/P3X-118/owncast/config.BuildPlatform=$NAME" -o owncast .
